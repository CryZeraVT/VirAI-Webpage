...
- Key/value store. Known keys:
  ...
  - `virforge_launcher_catalog` — VirForge stream catalog. `products.airi` has flat Release fields plus `channels.stable` / `channels.beta`. Paying keys stream Release; `tier=beta` streams Beta (falls back to Release). `get_virforge_catalog()` overlays the active `r2_versions` row per channel (website Storage activate is source of truth). `trg_sync_virforge_catalog_from_r2` writes that overlay back when a version is marked active. `trg_notify_virforge_catalog_updated` then `realtime.send(..., 'updated', 'virforge-catalog', false)` so running launchers refresh; they also poll every 2 minutes if the socket drops. Updated 2026-09-11.
