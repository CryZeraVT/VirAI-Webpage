...
- Key/value store. Known keys:
  ...
  - `virforge_launcher_catalog` — VirForge stream catalog. `products.airi` has flat Release fields plus `channels.stable` / `channels.beta`. Paying keys stream Release; `tier=beta` streams Beta (falls back to Release). `get_virforge_catalog()` fills `licenseEndpoint` if blank and copies a legacy flat URL into `channels.stable`. `publishedAt` is stamped on Apply. Admin broadcasts `virforge-catalog` / `updated` so running launchers recheck immediately; they also poll every 2 minutes if the socket drops. Updated 2026-09-10.
