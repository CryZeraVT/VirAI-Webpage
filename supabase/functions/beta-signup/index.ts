...
async function sendResendEmail(to: string | string[], subject: string, text: string) {
  if (!resendApiKey) {
...
serve(async (req) => {
  ...
  // Send acknowledgment to applicant (fire-and-forget)
  sendResendEmail(
    email.trim(),
    ...
    [
      `Hey ${name.trim()},`,
      "",
      "Thanks for applying to the ViriTTS beta! We're excited you want to try it out.",
      "",
      "We're reviewing applications and will reach out when your spot is ready.",
